...
namespace _20191121_SP
{
    public partial class FrmSistemaSolar : Form
    {
        ...
        private void FrmSistemaSolar_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Problemas con los archivos");
            }
        }
        ...
        private void btnSimular_Click(object sender, EventArgs e)
        {
            if(this.animaciones.Count == 0)
            {
                ...
                {
                    Thread hilo = new Thread(item.AnimarSistemaSolar);
        ...
        private void LimpiarAnimaciones()
        {
            ...
            foreach (Thread item in this.animaciones)
            {
                if (item.IsAlive)
                {
                    item.Abort();
        ...
        private void DibujarAvancePlaneta(object sender, PlanetaEventArgs e)
        {
            ...
            {
                InformacionDeAvance info = new InformacionDeAvance(DibujarAvancePlaneta);
                object[] arrayObjetos = new object[] { sender, e};
                pic.Invoke(info,arrayObjetos);
            }
            else
            {
